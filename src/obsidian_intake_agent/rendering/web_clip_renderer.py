...
def render_raw_web_clip_note(capture: WebClipCapture) -> str:
    ...
    return (
        ...
        "status: unprocessed\n"
        f"captured_at: {_yaml_scalar(capture.captured_at.isoformat())}\n"
        f"source_url: {_yaml_scalar(capture.source_url.strip())}\n"
        f"source_title: {_yaml_scalar(title)}\n"
        "---\n\n"
        f"# {title}\n\n"
...
def render_processed_web_clip_note(clip: ProcessedWebClip) -> str:
    topics = "\n".join(f"  - {_yaml_scalar(topic)}" for topic in clip.topics)
    related = "\n".join(f"  - {_yaml_scalar(item)}" for item in clip.related)
    ...
    return (
        "---\n"
        "type: web_clip\n"
        f"source_url: {_yaml_scalar(clip.source_url)}\n"
        f"source_title: {_yaml_scalar(clip.source_title)}\n"
        f"captured_at: {_yaml_scalar(clip.captured_at)}\n"
        "topics:\n"
        f"{topics_block}\n"
        "related:\n"
        f"{related_block}\n"
